...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul  8 16:58:18 2018
...
softmax = tf.nn.softmax(opt, name = "Softmax")
useless_layer = tf.nn.relu(softmax)
...
with tf.Session() as sess:
    ...
    for epochs in range(200):
        ...
        for i in range(len(features_train)):
            ...
            soft = sess.run(softmax, feed_dict = {x: features_train[i], y: [labels_train[i]]})
            useless = sess.run(useless_layer, feed_dict = {x: features_train[i], y: [labels_train[i]]})
    writer.close()
...
if __name__ == '__main__':
    ...
    freeze_graph(directory, "Softmax")